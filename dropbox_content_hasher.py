# https://raw.githubusercontent.com/dropbox/dropbox-api-content-hasher/master/python/dropbox_content_hasher.py
...
class DropboxContentHasher:
    ...
    def _finish(self) -> hashlib.sha256:
        if self._overall_hasher is None:
            raise AssertionError(
...
def db_hash(path: Path) -> str:
    ...
    with path.open(mode="rb") as f:
        while chunk := f.read(1024):  # or whatever chunk size you want
            hasher.update(chunk)
...
class StreamHasher:
    ...
    """
    def __init__(self, f: BinaryIO, hasher: Union[DropboxContentHasher, _hashlib.HASH]) -> None:
        self._f = f
        self._hasher = hasher
